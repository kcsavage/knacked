    ...
    if (userId !== knackevent.owner && ! _.contains(knackevent.invited, userId)) {
      ...
      if (Meteor.isServer && to) {
        Email.send({
          ...
          "\n\nCome check it out: " + Meteor.absoluteUrl() + "/knacktivity/" + knackeventId + "\n"
        });
      }
    }
  },
  inviteByEmail: function (knackeventId, email, userId) {
    var knackevent = knacktivity.findOne(knackeventId);
    if (! knackevent || knackevent.owner !== userId)
      throw new Meteor.Error(404, "No such knacktivity");
    if (userId == knackevent.owner) {
      var from = "tugboat@knacked.net"
      var to = email;
      if (Meteor.isServer && to) {
